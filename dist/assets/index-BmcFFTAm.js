var $=Object.defineProperty;var F=(c,r,e)=>r in c?$(c,r,{enumerable:!0,configurable:!0,writable:!0,value:e}):c[r]=e;var a=(c,r,e)=>F(c,typeof r!="symbol"?r+"":r,e);(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))t(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const n of i.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&t(n)}).observe(document,{childList:!0,subtree:!0});function e(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function t(s){if(s.ep)return;s.ep=!0;const i=e(s);fetch(s.href,i)}})();class M extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.setupEventListeners()}render(){this.shadowRoot&&(this.shadowRoot.innerHTML=`
    ...
    `)}setupEventListeners(){if(!this.shadowRoot)return;this.shadowRoot.querySelectorAll(".day-btn").forEach(e=>{e.addEventListener("click",t=>{const i=t.target.dataset.day;i&&this.selectDay(i)})}),this.shadowRoot.querySelector(".settings-btn")?.addEventListener("click",()=>{this.dispatchEvent(new CustomEvent("open-settings"))})}selectDay(r){if(!this.shadowRoot)return;this.shadowRoot.querySelectorAll(".day-btn").forEach(t=>{t.classList.remove("active")}),this.shadowRoot.querySelector(`.day-btn[data-day="${r}"]`)?.classList.add("active"),this.dispatchEvent(new CustomEvent("day-selected",{detail:{day:r}}))}showTimer(r,e){if(!this.shadowRoot)return;const t=this.shadowRoot.querySelector("#timer-bar"),s=this.shadowRoot.querySelector("#timer-display"),i=this.shadowRoot.querySelector("#timer-round");t&&t.classList.add("active"),s&&(s.textContent=r),i&&(i.textContent=e)}hideTimer(){if(!this.shadowRoot)return;const r=this.shadowRoot.querySelector("#timer-bar");r&&r.classList.remove("active")}}customElements.define("app-shell",M);const B="modulepreload",A=function(c){return"/"+c},D={},z=function(r,e,t){let s=Promise.resolve();if(e&&e.length>0){let u=function(h){return Promise.all(h.map(p=>Promise.resolve(p).then(l=>({status:"fulfilled",value:l}),l=>({status:"rejected",reason:l}))))};var n=u;document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),d=o?.nonce||o?.getAttribute("nonce");s=u(e.map(h=>{if(h=A(h),h in D)return;D[h]=!0;const p=h.endsWith(".css"),l=p?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${h}"]${l}`))return;const m=document.createElement("link");if(m.rel=p?"stylesheet":B,p||(m.as="script"),m.crossOrigin="",m.href=h,d&&m.setAttribute("nonce",d),document.head.appendChild(m),p)return new Promise((y,w)=>{m.addEventListener("load",y),m.addEventListener("error",()=>w(new Error(`Unable to preload CSS for ${h}`)))})}))}function i(o){const d=new Event("vite:preloadError",{cancelable:!0});if(d.payload=o,window.dispatchEvent(d),!d.defaultPrevented)throw o}return s.then(o=>{for(const d of o||[])d.status==="rejected"&&i(d.reason);return r().catch(i)})};class q{constructor(){a(this,"db",null);a(this,"dbName","minimalift_v1");a(this,"version",1)}async openDb(){return this.db?this.db:new Promise((r,e)=>{const t=indexedDB.open(this.dbName,this.version);t.onerror=()=>e(t.error),t.onsuccess=()=>{this.db=t.result,r(this.db)},t.onupgradeneeded=s=>{const i=s.target.result;if(i.objectStoreNames.contains("programs")||i.createObjectStore("programs",{keyPath:"programId"}),i.objectStoreNames.contains("days")||i.createObjectStore("days",{keyPath:"dayId"}).createIndex("programId","programId",{unique:!1}),!i.objectStoreNames.contains("sessions")){const n=i.createObjectStore("sessions",{keyPath:"sessionId",autoIncrement:!0});n.createIndex("dayId","dayId",{unique:!1}),n.createIndex("date","startedAt",{unique:!1})}}})}async tx(r,e="readonly"){return(await this.openDb()).transaction([r],e).objectStore(r)}async get(r,e){const t=await this.tx(r);return new Promise((s,i)=>{const n=t.get(e);n.onsuccess=()=>s(n.result),n.onerror=()=>i(n.error)})}async getAll(r){const e=await this.tx(r);return new Promise((t,s)=>{const i=e.getAll();i.onsuccess=()=>t(i.result),i.onerror=()=>s(i.error)})}async put(r,e){const t=await this.tx(r,"readwrite");return new Promise((s,i)=>{const n=t.put(e);n.onsuccess=()=>s(n.result),n.onerror=()=>i(n.error)})}async delete(r,e){const t=await this.tx(r,"readwrite");return new Promise((s,i)=>{const n=t.delete(e);n.onsuccess=()=>s(),n.onerror=()=>i(n.error)})}async indexGetAll(r,e,t){const i=(await this.tx(r)).index(e);return new Promise((n,o)=>{const d=t?i.getAll(t):i.getAll();d.onsuccess=()=>n(d.result),d.onerror=()=>o(d.error)})}async clear(r){const e=await this.tx(r,"readwrite");return new Promise((t,s)=>{const i=e.clear();i.onsuccess=()=>t(),i.onerror=()=>s(i.error)})}}const v=new q,x=class x{constructor(){}static getInstance(){return x.instance||(x.instance=new x),x.instance}async initialize(){const{seedData:r}=await z(async()=>{const{seedData:t}=await import("./seed-data-G69QiYQA.js");return{seedData:t}},[]);await v.get("programs",r.programId)||await this.loadSeedData(r)}async loadSeedData(r){const e={programId:r.programId,title:r.title};await v.put("programs",e);for(const t of r.days)await v.put("days",t);console.log("Seed data loaded successfully")}async getProgram(r){return await v.get("programs",r)}async getDaysForProgram(r){return await v.indexGetAll("days","programId",r)}async getDay(r){return await v.get("days",r)}async getAllDays(){return(await v.getAll("days")).sort((e,t)=>e.order-t.order)}};a(x,"instance");let P=x;const k=P.getInstance();class N extends HTMLElement{constructor(){super();a(this,"days",[]);this.attachShadow({mode:"open"})}async connectedCallback(){this.days=await k.getAllDays(),this.render()}render(){this.shadowRoot&&(this.shadowRoot.innerHTML=`
      <style>
        :host {
          display: block;
          padding: 16px;
    ...
    `}}customElements.define("view-home",N);class W extends HTMLElement{constructor(){super();a(this,"day",null);this.attachShadow({mode:"open"})}async loadDay(e){this.day=await k.getDay(e)||null,this.render()}render(){if(this.shadowRoot){if(!this.day){this.shadowRoot.innerHTML="<p>Loading...</p>";return}this.shadowRoot.innerHTML=`
      <style>
        :host {
          display: block;
          padding: 16px;
    ...
    `}renderCurrentExercises(){const e=this.getCurrentBlock();if(!e)return"";if(e.timerType==="interval"&&e.timerConfig?.exercisesPerInterval&&e.timerConfig.exercisesPerInterval>1){const s=e.timerConfig.exercisesPerInterval,i=Math.floor(this.currentExerciseIndex/s)*s;return`
        <div class="current-exercise">
          ${e.exercises.slice(i,i+s).map(o=>`
    ...
    `:""}renderTimer(){const e=this.getCurrentExercise(),t=this.getCurrentBlock(),s=t?.timerType&&t.timerType!=="none";if(t?.timerType==="interval"&&t.timerConfig?.exercisesPerInterval&&t.timerConfig.exercisesPerInterval>1){const p=(t.timerConfig.intervalSec||120)/60,l=t.timerConfig.exercisesPerInterval;if(this.currentTimer){const m=this.currentTimer.getRemainingInCurrentPeriod(),y=this.currentTimer.getCurrentRound(),w=this.currentTimer.getTotalRounds(),S=b.formatTime(m),T=this.getTimerProgress();return`
          <div class="timer-section">
            <div class="timer-display">${S}</div>
            <div class="timer-phase">Interval ${y} of ${w}</div>
            <div class="timer-progress">Complete all ${l} exercises • Every ${p} min</div>
        ...
        `}else return`
          <div class="timer-section">
            ...
            <div class="timer-progress">Every ${p} min • ${l} exercises per interval</div>
          </div>
        `}if(!s&&e){const p=e.sets||1,l=this.currentSetIndex+1;if(this.isRestingBetweenSets&&this.currentTimer){const m=this.currentTimer.getRemainingInCurrentPeriod(),y=b.formatTime(m),w=this.currentTimer.getElapsedTime(),S=this.currentTimer.getDuration(),T=Math.min(w/S,1);return`
          <div class="timer-section">
            <div class="timer-display">${y}</div>
            <div class="timer-phase">Rest after Set ${l-1}</div>
            <div class="timer-progress">Set ${l}/${p} next</div>
            <div class="progress-bar">
              <div class="progress-fill" style="width: ${T*100}%"></div>
            </div>
          </div>
        `}return`
        <div class="timer-section">
          <div class="timer-display">Set ${l}/${p}</div>
      ...
      `;const i=this.currentTimer.getRemainingInCurrentPeriod(),n=this.currentTimer.getCurrentRound(),o=this.currentTimer.getTotalRounds(),d=b.formatTime(i),u=this.getTimerProgress(),h=this.getTimerPhase();return`
      <div class="timer-section">
    ...
    `)}setupEventListeners(){if(!this.shadowRoot)return;const r=this.shadowRoot.querySelector("#update"),e=this.shadowRoot.querySelector("#dismiss");r?.addEventListener("click",()=>{this.applyUpdate()}),e?.addEventListener("click",()=>{this.hide()})}show(){this.classList.add("visible")}hide(){this.classList.remove("visible")}async applyUpdate(){const r=this.shadowRoot?.querySelector("#update");r&&(r.textContent="Updating...",r.disabled=!0),await L.applyUpdate()}}customElements.define("update-notification",X);class J{constructor(){a(this,"routes",[]);a(this,"currentPath","");window.addEventListener("hashchange",()=>this.handleRouteChange()),window.addEventListener("DOMContentLoaded",()=>this.handleRouteChange())}register(r,e){this.routes.push({path:r,handler:e})}navigate(r){window.location.hash=r}handleRouteChange(){const r=window.location.hash.slice(1)||"/";this.currentPath=r;const e=this.routes.find(t=>t.path.includes(":")?new RegExp("^"+t.path.replace(/:[^/]+/g,"([^/]+)")+"$").test(r):t.path===r);if(e)e.handler();else{const t=this.routes.find(s=>s.path==="/");t&&t.handler()}}getParams(){const r=window.location.hash.slice(1)||"/",e={};return this.routes.find(t=>{if(t.path.includes(":")){const s=t.path.split("/"),i=r.split("/");if(s.length===i.length){let n=!0;for(let o=0;o<s.length;o++)if(s[o].startsWith(":"))e[s[o].slice(1)]=i[o];else if(s[o]!==i[o]){n=!1;break}return n}}return!1}),e}getCurrentPath(){return this.currentPath}start(){this.handleRouteChange()}}const f=new J;document.addEventListener("DOMContentLoaded",async()=>{console.log("DOM loaded, initializing app...");try{let c=function(){t.innerHTML="<view-home></view-home>",t.querySelector("view-home")?.addEventListener("navigate-to-day",n=>{f.navigate(`/day/${n.detail.dayId}`)})},r=function(){const n=f.getParams().id;if(n){t.innerHTML="<view-day></view-day>";const o=t.querySelector("view-day");o&&o.loadDay&&o.loadDay(n),o?.addEventListener("start-session",d=>{f.navigate(`/session/${d.detail.dayId}`)})}};console.log("Initializing program manager..."),await k.initialize(),console.log("Program manager initialized");const e=document.querySelector("app-shell");if(!e){console.error("App shell not found");return}console.log("Found app shell, waiting for shadow DOM..."),await new Promise(i=>{const n=()=>{const o=e.shadowRoot;o&&o.querySelector("#content")?(console.log("Shadow DOM ready"),i(void 0)):setTimeout(n,10)};n()});const t=e.shadowRoot.querySelector("#content");if(!t){console.error("Content container not found");return}f.register("/",()=>c()),f.register("/day/:id",()=>r()),f.register("/session/:id",()=>s());async function s(){const n=f.getParams().id;if(n){const o=await k.getDay(n);if(o){t.innerHTML="<view-session></view-session>";const d=t.querySelector("view-session");d&&d.setDay&&d.setDay(o),d?.addEventListener("session-complete",u=>{console.log("Session completed:",u.detail),f.navigate(`/day/${n}`)})}}}e&&(e.addEventListener("day-selected",i=>{const o=`p1_w1_d${i.detail.day}`;f.navigate(`/day/${o}`)}),e.addEventListener("open-settings",()=>{console.log("Open settings")})),console.log("Starting router..."),f.start(),!window.location.hash||window.location.hash==="#/"?(console.log("No hash found, navigating to Day 1"),f.navigate("/day/p1_w1_d1")):console.log("Found hash:",window.location.hash),console.log("App initialization complete")}catch(c){console.error("Failed to initialize app:",c),document.body.innerHTML=`
    ...
    `}});
//# sourceMappingURL=index-BmcFFTAm.js.map
