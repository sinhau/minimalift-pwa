var L=Object.defineProperty;var F=(c,t,e)=>t in c?L(c,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):c[t]=e;var a=(c,t,e)=>F(c,typeof t!="symbol"?t+"":t,e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))r(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const n of i.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&r(n)}).observe(document,{childList:!0,subtree:!0});function e(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function r(s){if(s.ep)return;s.ep=!0;const i=e(s);fetch(s.href,i)}})();class $ extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.setupEventListeners()}render(){this.shadowRoot&&(this.shadowRoot.innerHTML=`
    ...
    `}renderControls(){const e=this.getCurrentBlock();if(!(e?.timerMode&&e.timerMode!=="none"))return`
      ...
      `;const s=this.currentTimer?.getState()||"idle",i=s==="completed";let n="";return!this.currentTimer||s==="idle"?n='<button class="control-btn primary" id="start-btn">Start Timer</button>':s==="running"?n='<button class="control-btn warning" id="pause-btn">Pause</button>':s==="paused"?n='<button class="control-btn primary" id="resume-btn">Resume</button>':i&&(n='<button class="control-btn primary" id="next-btn">Next Exercise</button>'),`
      <div class="session-controls">
        ${n}
        <button class="control-btn secondary" id="reset-btn" ${!this.currentTimer||s==="idle"?"disabled":""}>Reset</button>
        <button class="control-btn secondary" id="skip-btn">Skip</button>
      </div>
    ...
    `)}setupEventListeners(){if(!this.shadowRoot)return;const t=this.shadowRoot.querySelector("#update"),e=this.shadowRoot.querySelector("#dismiss");t?.addEventListener("click",()=>{this.applyUpdate()}),e?.addEventListener("click",()=>{this.hide()})}show(){this.classList.add("visible")}hide(){this.classList.remove("visible")}async applyUpdate(){const t=this.shadowRoot?.querySelector("#update");t&&(t.textContent="Updating...",t.disabled=!0),await M.applyUpdate()}}customElements.define("update-notification",Y);class G{constructor(){a(this,"routes",[]);a(this,"currentPath","");window.addEventListener("hashchange",()=>this.handleRouteChange()),window.addEventListener("DOMContentLoaded",()=>this.handleRouteChange())}register(t,e){this.routes.push({path:t,handler:e})}navigate(t){window.location.hash=t}handleRouteChange(){const t=window.location.hash.slice(1)||"/";this.currentPath=t;const e=this.routes.find(r=>r.path.includes(":")?new RegExp("^"+r.path.replace(/:[^/]+/g,"([^/]+)")+"$").test(t):r.path===t);if(e)e.handler();else{const r=this.routes.find(s=>s.path==="/");r&&r.handler()}}getParams(){const t=window.location.hash.slice(1)||"/",e={};return this.routes.find(r=>{if(r.path.includes(":")){const s=r.path.split("/"),i=t.split("/");if(s.length===i.length){let n=!0;for(let o=0;o<s.length;o++)if(s[o].startsWith(":"))e[s[o].slice(1)]=i[o];else if(s[o]!==i[o]){n=!1;break}return n}}return!1}),e}getCurrentPath(){return this.currentPath}start(){this.handleRouteChange()}}const l=new G;document.addEventListener("DOMContentLoaded",async()=>{console.log("DOM loaded, initializing app...");try{let c=function(){r.innerHTML="<view-home></view-home>",r.querySelector("view-home")?.addEventListener("navigate-to-day",n=>{l.navigate(`/day/${n.detail.dayId}`)})},t=function(){const n=l.getParams().id;if(n){r.innerHTML="<view-day></view-day>";const o=r.querySelector("view-day");o&&o.loadDay&&o.loadDay(n),o?.addEventListener("start-session",d=>{l.navigate(`/session/${d.detail.dayId}`)})}};console.log("Initializing program manager..."),await k.initialize(),console.log("Program manager initialized");const e=document.querySelector("app-shell");if(!e){console.error("App shell not found");return}console.log("Found app shell, waiting for shadow DOM..."),await new Promise(i=>{const n=()=>{const o=e.shadowRoot;o&&o.querySelector("#content")?(console.log("Shadow DOM ready"),i(void 0)):setTimeout(n,10)};n()});const r=e.shadowRoot.querySelector("#content");if(!r){console.error("Content container not found");return}l.register("/",()=>c()),l.register("/day/:id",()=>t()),l.register("/session/:id",()=>s());async function s(){const n=l.getParams().id;if(n){const o=await k.getDay(n);if(o){r.innerHTML="<view-session></view-session>";const d=r.querySelector("view-session");d&&d.setDay&&d.setDay(o),d?.addEventListener("session-complete",x=>{console.log("Session completed:",x.detail),l.navigate(`/day/${n}`)})}}}e&&(e.addEventListener("day-selected",i=>{const o=`p1_w1_d${i.detail.day}`;l.navigate(`/day/${o}`)}),e.addEventListener("open-settings",()=>{console.log("Open settings")})),console.log("Starting router..."),l.start(),!window.location.hash||window.location.hash==="#/"?(console.log("No hash found, navigating to Day 1"),l.navigate("/day/p1_w1_d1")):console.log("Found hash:",window.location.hash),console.log("App initialization complete")}catch(c){console.error("Failed to initialize app:",c),document.body.innerHTML=`
    ...
    `}});
//# sourceMappingURL=index-Bg8SUAf0.js.map
