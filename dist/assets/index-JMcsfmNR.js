var $=Object.defineProperty;var F=(c,i,t)=>i in c?$(c,i,{enumerable:!0,configurable:!0,writable:!0,value:t}):c[i]=t;var a=(c,i,t)=>F(c,typeof i!="symbol"?i+"":i,t);(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))e(s);new MutationObserver(s=>{for(const r of s)if(r.type==="childList")for(const n of r.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&e(n)}).observe(document,{childList:!0,subtree:!0});function t(s){const r={};return s.integrity&&(r.integrity=s.integrity),s.referrerPolicy&&(r.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?r.credentials="include":s.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function e(s){if(s.ep)return;s.ep=!0;const r=t(s);fetch(s.href,r)}})();class M extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.setupEventListeners()}render(){this.shadowRoot&&(this.shadowRoot.innerHTML=`
    ...
    `}}customElements.define("view-home",N);class W extends HTMLElement{constructor(){super();a(this,"day",null);this.attachShadow({mode:"open"})}async loadDay(t){this.day=await k.getDay(t)||null,this.render()}render(){if(this.shadowRoot){if(!this.day){this.shadowRoot.innerHTML="<p>Loading...</p>";return}this.shadowRoot.innerHTML=`
      <style>
        :host {
          display: block;
          padding: 16px;
    ...
    `}renderCurrentExercise(t){const e=this.getCurrentBlock();if(!e)return"";if(e.timerType==="interval"&&e.timerConfig?.exercisesPerInterval&&e.timerConfig.exercisesPerInterval>1){const s=e.timerConfig.exercisesPerInterval;return`
        <div class="current-exercise">
          ${e.exercises.slice(0,s).map(n=>`
    ...
    `}renderTimer(){const t=this.getCurrentExercise(),e=this.getCurrentBlock(),s=e?.timerType&&e.timerType!=="none";if(e?.timerType==="interval"&&e.timerConfig?.exercisesPerInterval&&e.timerConfig.exercisesPerInterval>1){const l=(e.timerConfig.intervalSec||120)/60,m=e.timerConfig.exercisesPerInterval;if(this.currentTimer){const g=this.currentTimer.getRemainingInCurrentPeriod(),x=this.currentTimer.getCurrentRound(),w=this.currentTimer.getTotalRounds(),S=b.formatTime(g),T=this.getTimerProgress();return`
          <div class="timer-section">
            <div class="timer-display">${S}</div>
            <div class="timer-phase">Interval ${x} of ${w}</div>
            <div class="timer-progress">Complete all ${m} exercises • Every ${l} min</div>
        ...
        `}else return`
          <div class="timer-section">
            ...
            <div class="timer-progress">Every ${l} min • ${m} exercises per interval</div>
          </div>
        `}if(!s&&t){const l=t.sets||1,m=this.currentSetIndex+1;if(this.isRestingBetweenSets&&this.currentTimer){const g=this.currentTimer.getRemainingInCurrentPeriod(),x=b.formatTime(g),w=this.currentTimer.getElapsedTime(),S=this.currentTimer.getDuration(),T=Math.min(w/S,1);return`
          <div class="timer-section">
            <div class="timer-display">${x}</div>
            <div class="timer-phase">Rest after Set ${m-1}</div>
            <div class="timer-progress">Set ${m}/${l} next</div>
            <div class="progress-bar">
              <div class="progress-fill" style="width: ${T*100}%"></div>
            </div>
          </div>
        `}return`
        <div class="timer-section">
          <div class="timer-display">Set ${m}/${l}</div>
    ...
    `}getCurrentBlock(){return!this.currentDay||this.currentBlockIndex>=this.currentDay.blocks.length?null:this.currentDay.blocks[this.currentBlockIndex]}getCurrentExercise(){const t=this.getCurrentBlock();return!t||this.currentExerciseIndex>=t.exercises.length?null:t.exercises[this.currentExerciseIndex]}getNextExercise(){const t=this.getCurrentBlock();if(!t)return null;let e=1;return t.timerType==="interval"&&t.timerConfig?.exercisesPerInterval&&(e=t.timerConfig.exercisesPerInterval),this.currentExerciseIndex+e<t.exercises.length?t.exercises[this.currentExerciseIndex+e]:!this.currentDay||this.currentBlockIndex+1>=this.currentDay.blocks.length?null:this.currentDay.blocks[this.currentBlockIndex+1].exercises[0]||null}formatExerciseDetails(t,e){const s=[];t.sets&&s.push(`${t.sets} sets`),t.reps&&s.push(`${t.reps} reps`),t.restSec&&s.push(`${t.restSec}s rest`);const r=e.timerType;return r&&r!=="none"&&s.push(`${r.toUpperCase()} format`),s.join(" • ")}getTimerProgress(){if(!this.currentTimer)return 0;const t=this.currentTimer.getElapsedTime(),e=this.currentTimer.getDuration();return Math.min(t/e,1)}getTimerPhase(){if(!this.currentTimer)return"Ready";const t=this.getCurrentBlock();if(!(t?.timerType&&t.timerType!=="none"))return"Timer Running";if(this.currentTimer&&this.currentTimer.getTotalRounds()>1){const s=this.currentTimer.getCurrentPhase?.();if(s==="work")return"Work Period";if(s==="rest")return"Rest Period"}return"Timer Running"}createTimerForCurrentBlock(){const t=this.getCurrentBlock();return!t?.timerType||t.timerType==="none"?null:D.createTimer(t.timerType,t.timerConfig)}setupEventListeners(){this.shadowRoot&&this.shadowRoot.addEventListener("click",t=>{const e=t.target;e.id==="start-btn"?this.startTimer():e.id==="pause-btn"?this.pauseTimer():e.id==="resume-btn"?this.resumeTimer():e.id==="reset-btn"?this.resetTimer():e.id==="skip-btn"?this.skipExercise():e.id==="complete-exercise-btn"?this.completeExercise():e.id==="complete-set-btn"?this.completeSet():e.id==="skip-rest-btn"?this.skipRest():e.id==="next-set-btn"?this.startNextSet():e.id==="next-btn"?this.nextExercise():e.id==="finish-btn"?this.finishSession():e.id==="cancel-session"&&this.cancelSession()})}async startTimer(){this.sessionStartTime||(this.sessionStartTime=performance.now(),await I.requestNotificationPermission()),await R.acquire(),this.currentTimer=this.createTimerForCurrentBlock(),this.currentTimer&&(this.currentTimer.addCallback(t=>{this.handleTimerEvent(t)}),this.currentTimer.start(),p.timerStart(),this.render())}pauseTimer(){this.currentTimer?.pause(),p.timerPause(),this.render()}resumeTimer(){this.currentTimer?.start(),p.timerStart(),this.render()}resetTimer(){this.currentTimer?.reset(),this.render()}skipExercise(){this.nextExercise()}completeExercise(){this.nextExercise()}async completeSet(){const t=this.getCurrentExercise();if(!t)return;const e=t.sets||1,s=this.currentSetIndex>=e-1;p.setComplete(),s?this.nextExercise():(t.restSec&&t.restSec>0?(this.currentSetIndex++,this.isRestingBetweenSets=!0,this.currentTimer=D.createRestTimer(t.restSec,1),this.currentTimer.addCallback(r=>{r.type==="complete"?this.handleRestComplete():r.type==="tick"&&this.updateTimerDisplay()}),this.currentTimer.start(),p.restStart()):this.currentSetIndex++,this.render())}skipRest(){this.currentTimer&&(this.currentTimer.stop(),this.currentTimer=null),this.handleRestComplete()}startNextSet(){this.handleRestComplete()}handleRestComplete(){this.isRestingBetweenSets=!1,this.currentTimer?.stop(),this.currentTimer=null,p.restComplete(),this.render()}nextExercise(){const t=this.getCurrentBlock();if(!t)return;p.exerciseComplete(),this.currentTimer?.stop(),this.currentTimer=null,this.currentSetIndex=0,this.isRestingBetweenSets=!1;let e=1;t.timerType==="interval"&&t.timerConfig?.exercisesPerInterval&&(e=t.timerConfig.exercisesPerInterval),this.currentExerciseIndex+e<t.exercises.length?this.currentExerciseIndex+=e:(this.currentBlockIndex++,this.currentExerciseIndex=0),this.render()}async finishSession(){p.sessionComplete(),await this.cleanup(),this.showSessionSaveDialog()}showSessionSaveDialog(){const t=this.sessionStartTime?Math.round((performance.now()-this.sessionStartTime)/1e3):0,e=Math.floor(t/60),s=t%60,r=`${e}:${s.toString().padStart(2,"0")}`;if(!this.shadowRoot)return;const n=`
    ...
    `)}setupEventListeners(){if(!this.shadowRoot)return;const i=this.shadowRoot.querySelector("#update"),t=this.shadowRoot.querySelector("#dismiss");i?.addEventListener("click",()=>{this.applyUpdate()}),t?.addEventListener("click",()=>{this.hide()})}show(){this.classList.add("visible")}hide(){this.classList.remove("visible")}async applyUpdate(){const i=this.shadowRoot?.querySelector("#update");i&&(i.textContent="Updating...",i.disabled=!0),await L.applyUpdate()}}customElements.define("update-notification",X);class J{constructor(){a(this,"routes",[]);a(this,"currentPath","");window.addEventListener("hashchange",()=>this.handleRouteChange()),window.addEventListener("DOMContentLoaded",()=>this.handleRouteChange())}register(i,t){this.routes.push({path:i,handler:t})}navigate(i){window.location.hash=i}handleRouteChange(){const i=window.location.hash.slice(1)||"/";this.currentPath=i;const t=this.routes.find(e=>e.path.includes(":")?new RegExp("^"+e.path.replace(/:[^/]+/g,"([^/]+)")+"$").test(i):e.path===i);if(t)t.handler();else{const e=this.routes.find(s=>s.path==="/");e&&e.handler()}}getParams(){const i=window.location.hash.slice(1)||"/",t={};return this.routes.find(e=>{if(e.path.includes(":")){const s=e.path.split("/"),r=i.split("/");if(s.length===r.length){let n=!0;for(let o=0;o<s.length;o++)if(s[o].startsWith(":"))t[s[o].slice(1)]=r[o];else if(s[o]!==r[o]){n=!1;break}return n}}return!1}),t}getCurrentPath(){return this.currentPath}start(){this.handleRouteChange()}}const f=new J;document.addEventListener("DOMContentLoaded",async()=>{console.log("DOM loaded, initializing app...");try{let c=function(){e.innerHTML="<view-home></view-home>",e.querySelector("view-home")?.addEventListener("navigate-to-day",n=>{f.navigate(`/day/${n.detail.dayId}`)})},i=function(){const n=f.getParams().id;if(n){e.innerHTML="<view-day></view-day>";const o=e.querySelector("view-day");o&&o.loadDay&&o.loadDay(n),o?.addEventListener("start-session",d=>{f.navigate(`/session/${d.detail.dayId}`)})}};console.log("Initializing program manager..."),await k.initialize(),console.log("Program manager initialized");const t=document.querySelector("app-shell");if(!t){console.error("App shell not found");return}console.log("Found app shell, waiting for shadow DOM..."),await new Promise(r=>{const n=()=>{const o=t.shadowRoot;o&&o.querySelector("#content")?(console.log("Shadow DOM ready"),r(void 0)):setTimeout(n,10)};n()});const e=t.shadowRoot.querySelector("#content");if(!e){console.error("Content container not found");return}f.register("/",()=>c()),f.register("/day/:id",()=>i()),f.register("/session/:id",()=>s());async function s(){const n=f.getParams().id;if(n){const o=await k.getDay(n);if(o){e.innerHTML="<view-session></view-session>";const d=e.querySelector("view-session");d&&d.setDay&&d.setDay(o),d?.addEventListener("session-complete",u=>{console.log("Session completed:",u.detail),f.navigate(`/day/${n}`)})}}}t&&(t.addEventListener("day-selected",r=>{const o=`p1_w1_d${r.detail.day}`;f.navigate(`/day/${o}`)}),t.addEventListener("open-settings",()=>{console.log("Open settings")})),console.log("Starting router..."),f.start(),!window.location.hash||window.location.hash==="#/"?(console.log("No hash found, navigating to Day 1"),f.navigate("/day/p1_w1_d1")):console.log("Found hash:",window.location.hash),console.log("App initialization complete")}catch(c){console.error("Failed to initialize app:",c),document.body.innerHTML=`
    ...
    `}});
//# sourceMappingURL=index-JMcsfmNR.js.map
