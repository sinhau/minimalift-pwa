var L=Object.defineProperty;var $=(c,t,e)=>t in c?L(c,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):c[t]=e;var o=(c,t,e)=>$(c,typeof t!="symbol"?t+"":t,e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))r(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const n of i.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&r(n)}).observe(document,{childList:!0,subtree:!0});function e(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function r(s){if(s.ep)return;s.ep=!0;const i=e(s);fetch(s.href,i)}})();class A extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.setupEventListeners()}render(){this.shadowRoot&&(this.shadowRoot.innerHTML=`
      <style>
        :host {
          display: flex;
    ...
    `)}setupEventListeners(){if(!this.shadowRoot)return;this.shadowRoot.querySelectorAll(".day-btn").forEach(r=>{r.addEventListener("click",s=>{const n=s.target.dataset.day;n&&this.selectDay(n)})});const t=this.shadowRoot.querySelector("#refresh-btn");t?.addEventListener("click",async()=>{t.classList.add("loading");const r=this.shadowRoot?.querySelector("update-notification");r&&r.checkForUpdates&&await r.checkForUpdates(),setTimeout(()=>{t.classList.remove("loading")},1e3)}),this.shadowRoot.querySelector(".settings-btn")?.addEventListener("click",()=>{this.dispatchEvent(new CustomEvent("open-settings"))})}selectDay(t){if(!this.shadowRoot)return;this.shadowRoot.querySelectorAll(".day-btn").forEach(r=>{r.classList.remove("active")}),this.shadowRoot.querySelector(`.day-btn[data-day="${t}"]`)?.classList.add("active"),this.dispatchEvent(new CustomEvent("day-selected",{detail:{day:t}}))}showTimer(t,e){if(!this.shadowRoot)return;const r=this.shadowRoot.querySelector("#timer-bar"),s=this.shadowRoot.querySelector("#timer-display"),i=this.shadowRoot.querySelector("#timer-round");r&&r.classList.add("active"),s&&(s.textContent=t),i&&(i.textContent=e)}hideTimer(){if(!this.shadowRoot)return;const t=this.shadowRoot.querySelector("#timer-bar");t&&t.classList.remove("active")}}customElements.define("app-shell",A);const F="modulepreload",B=function(c){return"/"+c},D={},q=function(t,e,r){let s=Promise.resolve();if(e&&e.length>0){let w=function(l){return Promise.all(l.map(x=>Promise.resolve(x).then(v=>({status:"fulfilled",value:v}),v=>({status:"rejected",reason:v}))))};var n=w;document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),d=a?.nonce||a?.getAttribute("nonce");s=w(e.map(l=>{if(l=B(l),l in D)return;D[l]=!0;const x=l.endsWith(".css"),v=x?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${v}`))return;const p=document.createElement("link");if(p.rel=x?"stylesheet":F,x||(p.as="script"),p.crossOrigin="",p.href=l,d&&p.setAttribute("nonce",d),document.head.appendChild(p),x)return new Promise((R,I)=>{p.addEventListener("load",R),p.addEventListener("error",()=>I(new Error(`Unable to preload CSS for ${l}`)))})}))}function i(a){const d=new Event("vite:preloadError",{cancelable:!0});if(d.payload=a,window.dispatchEvent(d),!d.defaultPrevented)throw a}return s.then(a=>{for(const d of a||[])d.status==="rejected"&&i(d.reason);return t().catch(i)})};class z{constructor(){o(this,"db",null);o(this,"dbName","minimalift_v1");o(this,"version",1)}async openDb(){return this.db?this.db:new Promise((t,e)=>{const r=indexedDB.open(this.dbName,this.version);r.onerror=()=>e(r.error),r.onsuccess=()=>{this.db=r.result,t(this.db)},r.onupgradeneeded=s=>{const i=s.target.result;if(i.objectStoreNames.contains("programs")||i.createObjectStore("programs",{keyPath:"programId"}),i.objectStoreNames.contains("days")||i.createObjectStore("days",{keyPath:"dayId"}).createIndex("programId","programId",{unique:!1}),!i.objectStoreNames.contains("sessions")){const n=i.createObjectStore("sessions",{keyPath:"sessionId",autoIncrement:!0});n.createIndex("dayId","dayId",{unique:!1}),n.createIndex("date","startedAt",{unique:!1})}}})}async tx(t,e="readonly"){return(await this.openDb()).transaction([t],e).objectStore(t)}async get(t,e){const r=await this.tx(t);return new Promise((s,i)=>{const n=r.get(e);n.onsuccess=()=>s(n.result),n.onerror=()=>i(n.error)})}async getAll(t){const e=await this.tx(t);return new Promise((r,s)=>{const i=e.getAll();i.onsuccess=()=>r(i.result),i.onerror=()=>s(i.error)})}async put(t,e){const r=await this.tx(t,"readwrite");return new Promise((s,i)=>{const n=r.put(e);n.onsuccess=()=>s(n.result),n.onerror=()=>i(n.error)})}async delete(t,e){const r=await this.tx(t,"readwrite");return new Promise((s,i)=>{const n=r.delete(e);n.onsuccess=()=>s(),n.onerror=()=>i(n.error)})}async indexGetAll(t,e,r){const i=(await this.tx(t)).index(e);return new Promise((n,a)=>{const d=r?i.getAll(r):i.getAll();d.onsuccess=()=>n(d.result),d.onerror=()=>a(d.error)})}async clear(t){const e=await this.tx(t,"readwrite");return new Promise((r,s)=>{const i=e.clear();i.onsuccess=()=>r(),i.onerror=()=>s(i.error)})}}const g=new z,f=class f{constructor(){}static getInstance(){return f.instance||(f.instance=new f),f.instance}async initialize(){const{seedData:t}=await q(async()=>{const{seedData:r}=await import("./seed-data-C_uM7Nqm.js");return{seedData:r}},[]);await g.get("programs",t.programId)||await this.loadSeedData(t)}async loadSeedData(t){const e={programId:t.programId,title:t.title};await g.put("programs",e);for(const r of t.days)await g.put("days",r);console.log("Seed data loaded successfully")}async getProgram(t){return await g.get("programs",t)}async getDaysForProgram(t){return await g.indexGetAll("days","programId",t)}async getDay(t){return await g.get("days",t)}async getAllDays(){return(await g.getAll("days")).sort((e,r)=>e.order-r.order)}};o(f,"instance");let M=f;const k=M.getInstance();class U extends HTMLElement{constructor(){super();o(this,"days",[]);this.attachShadow({mode:"open"})}async connectedCallback(){this.days=await k.getAllDays(),this.render()}render(){this.shadowRoot&&(this.shadowRoot.innerHTML=`
      <style>
        :host {
          display: block;
          padding: 16px;
    ...
    `}}customElements.define("view-home",U);class N extends HTMLElement{constructor(){super();o(this,"day",null);this.attachShadow({mode:"open"})}async loadDay(e){this.day=await k.getDay(e)||null,this.render()}render(){if(this.shadowRoot){if(!this.day){this.shadowRoot.innerHTML="<p>Loading...</p>";return}this.shadowRoot.innerHTML=`
      <style>
        :host {
          display: block;
          padding: 16px;
    ...
    `:""}renderTimer(){if(!this.currentTimer)return`
      ...
      `;const e=this.currentTimer.getRemainingInCurrentPeriod(),r=this.currentTimer.getCurrentRound(),s=this.currentTimer.getTotalRounds(),i=b.formatTime(e),n=this.getTimerProgress(),a=this.getTimerPhase();return`
      <div class="timer-section">
        <div class="timer-display">${i}</div>
        <div class="timer-phase">${a}</div>
    ...
    `}getCurrentBlock(){return!this.currentDay||this.currentBlockIndex>=this.currentDay.blocks.length?null:this.currentDay.blocks[this.currentBlockIndex]}getCurrentExercise(){const e=this.getCurrentBlock();return!e||this.currentExerciseIndex>=e.exercises.length?null:e.exercises[this.currentExerciseIndex]}getNextExercise(){const e=this.getCurrentBlock();return e?this.currentExerciseIndex+1<e.exercises.length?e.exercises[this.currentExerciseIndex+1]:!this.currentDay||this.currentBlockIndex+1>=this.currentDay.blocks.length?null:this.currentDay.blocks[this.currentBlockIndex+1].exercises[0]||null:null}formatExerciseDetails(e,r){const s=[];return e.sets&&s.push(`${e.sets} sets`),e.reps&&s.push(`${e.reps} reps`),e.restSec&&s.push(`${e.restSec}s rest`),r.timerMode&&r.timerMode!=="none"&&s.push(`${r.timerMode.toUpperCase()} format`),s.join(" • ")}getTimerProgress(){if(!this.currentTimer)return 0;const e=this.currentTimer.getElapsedTime(),r=this.currentTimer.getDuration();return Math.min(e/r,1)}getTimerPhase(){if(!this.currentTimer)return"Ready";const e=this.getCurrentBlock();if(!e?.timerMode||e.timerMode==="none")return"Timer Running";if(this.currentTimer instanceof T||this.currentTimer instanceof y){const r=this.currentTimer.getCurrentPhase?.();if(r==="work")return"Work Period";if(r==="rest")return"Rest Period"}return"Timer Running"}createTimerForCurrentBlock(){const e=this.getCurrentBlock();if(!e?.timerMode||e.timerMode==="none")return null;const r=e.rounds||e.exercises.length;switch(e.timerMode){case"emom":return m.createEMOM(r);case"e2mom":return m.createE2MOM(r);case"e4mom":return m.createE4MOM(r);case"n90":return T.create(r);case"timed_circuit":return y.createTabata(r);case"fixed_rest":const s=e.durationSec||180;return C.create(s);default:return null}}setupEventListeners(){this.shadowRoot&&this.shadowRoot.addEventListener("click",e=>{const r=e.target;r.id==="start-btn"?this.startTimer():r.id==="pause-btn"?this.pauseTimer():r.id==="resume-btn"?this.resumeTimer():r.id==="reset-btn"?this.resetTimer():r.id==="skip-btn"?this.skipExercise():r.id==="next-btn"?this.nextExercise():r.id==="finish-btn"&&this.finishSession()})}async startTimer(){this.sessionStartTime||(this.sessionStartTime=performance.now(),await S.requestNotificationPermission()),await P.acquire(),this.currentTimer=this.createTimerForCurrentBlock(),this.currentTimer&&(this.currentTimer.addCallback(e=>{this.handleTimerEvent(e)}),this.currentTimer.start(),u.timerStart(),this.render())}pauseTimer(){this.currentTimer?.pause(),u.timerPause(),this.render()}resumeTimer(){this.currentTimer?.start(),u.timerStart(),this.render()}resetTimer(){this.currentTimer?.reset(),this.render()}skipExercise(){this.nextExercise()}nextExercise(){const e=this.getCurrentBlock();e&&(u.exerciseComplete(),this.currentTimer?.stop(),this.currentTimer=null,this.currentExerciseIndex+1<e.exercises.length?this.currentExerciseIndex++:(this.currentBlockIndex++,this.currentExerciseIndex=0),this.render())}async finishSession(){u.sessionComplete(),await this.cleanup(),this.dispatchEvent(new CustomEvent("session-complete",{detail:{day:this.currentDay,duration:this.sessionStartTime?performance.now()-this.sessionStartTime:0}}))}setupBackgroundHandling(){this.foregroundCallback=()=>{this.currentTimer?.getState()==="running"&&P.reacquire(),this.updateTimerDisplay()},S.addForegroundCallback(this.foregroundCallback)}async cleanup(){this.currentTimer&&(this.currentTimer.stop(),this.currentTimer=null),await P.release(),this.foregroundCallback&&(S.removeForegroundCallback(this.foregroundCallback),this.foregroundCallback=null)}handleTimerEvent(e){e.type==="tick"?(this.updateTimerDisplay(),this.checkForPhaseChange(),this.checkForCountdownWarning(e.remaining)):e.type==="complete"?(u.exerciseComplete(),this.render()):e.type==="roundComplete"&&u.roundComplete()}checkForPhaseChange(){const e=this.getTimerPhase();this.lastPhase&&this.lastPhase!==e&&(e==="Work Period"?u.workPeriodStart():e==="Rest Period"&&u.restPeriodStart()),this.lastPhase=e}checkForCountdownWarning(e){const r=Math.ceil(e/1e3);if(r<=3&&r>0){const s=this.lastWarnTime||0,i=performance.now();i-s>900&&(u.countdownWarning(),this.lastWarnTime=i)}}updateTimerDisplay(){if(!this.shadowRoot||!this.currentTimer)return;const e=this.shadowRoot.querySelector(".timer-display"),r=this.shadowRoot.querySelector(".progress-fill"),s=this.shadowRoot.querySelector(".timer-progress"),i=this.shadowRoot.querySelector(".timer-phase");if(e){const n=this.currentTimer.getRemainingInCurrentPeriod();e.textContent=b.formatTime(n)}if(r){const n=this.getTimerProgress();r.style.width=`${n*100}%`}if(s){const n=this.currentTimer.getCurrentRound(),a=this.currentTimer.getTotalRounds();s.textContent=`Round ${n} of ${a}`}i&&(i.textContent=this.getTimerPhase())}}customElements.define("view-session",j);class V{constructor(){o(this,"registration",null);o(this,"showUpdateCallback",null);document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>this.setupUpdateDetection()):this.setupUpdateDetection()}async setupUpdateDetection(){if(!("serviceWorker"in navigator)){console.log("Service workers not supported");return}try{this.registration=await navigator.serviceWorker.register("/sw.js"),console.log("ServiceWorker registered:",this.registration),await this.checkForUpdates(),setInterval(()=>{document.visibilityState==="visible"&&this.checkForUpdates()},3e4),this.registration.addEventListener("updatefound",()=>{console.log("Update found, installing new version...");const t=this.registration?.installing;t&&t.addEventListener("statechange",()=>{t.state==="installed"&&navigator.serviceWorker.controller&&(console.log("New version ready"),this.showUpdateAvailable())})}),navigator.serviceWorker.addEventListener("controllerchange",()=>{console.log("New service worker took control, reloading page"),window.location.reload()})}catch(t){console.error("ServiceWorker registration failed:",t)}}async checkForUpdates(){if(this.registration)try{await this.registration.update()}catch(t){console.error("Failed to check for updates:",t)}}showUpdateAvailable(){this.showUpdateCallback?this.showUpdateCallback():this.showDefaultUpdateNotification()}showDefaultUpdateNotification(){confirm("A new version of Minimalift is available! Click OK to update now.")&&this.applyUpdate()}onUpdateAvailable(t){this.showUpdateCallback=t}async applyUpdate(){if(!this.registration?.waiting){console.log("No update waiting");return}this.registration.waiting.postMessage({type:"SKIP_WAITING"})}async forceUpdateCheck(){console.log("Forcing update check..."),await this.checkForUpdates()}}const E=new V;class G extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.setupUpdateListener()}connectedCallback(){this.render(),this.setupEventListeners()}setupUpdateListener(){E.onUpdateAvailable(()=>{this.show()})}render(){this.shadowRoot&&(this.shadowRoot.innerHTML=`
      <style>
        :host {
          position: fixed;
          bottom: 0;
    ...
    `)}setupEventListeners(){if(!this.shadowRoot)return;const t=this.shadowRoot.querySelector("#update"),e=this.shadowRoot.querySelector("#dismiss");t?.addEventListener("click",()=>{this.applyUpdate()}),e?.addEventListener("click",()=>{this.hide()})}show(){this.classList.add("visible")}hide(){this.classList.remove("visible")}async applyUpdate(){const t=this.shadowRoot?.querySelector("#update");t&&(t.textContent="Updating...",t.disabled=!0),await E.applyUpdate()}async checkForUpdates(){await E.forceUpdateCheck()}}customElements.define("update-notification",G);class Y{constructor(){o(this,"routes",[]);o(this,"currentPath","");window.addEventListener("hashchange",()=>this.handleRouteChange()),window.addEventListener("DOMContentLoaded",()=>this.handleRouteChange())}register(t,e){this.routes.push({path:t,handler:e})}navigate(t){window.location.hash=t}handleRouteChange(){const t=window.location.hash.slice(1)||"/";this.currentPath=t;const e=this.routes.find(r=>r.path.includes(":")?new RegExp("^"+r.path.replace(/:[^/]+/g,"([^/]+)")+"$").test(t):r.path===t);if(e)e.handler();else{const r=this.routes.find(s=>s.path==="/");r&&r.handler()}}getParams(){const t=window.location.hash.slice(1)||"/",e={};return this.routes.find(r=>{if(r.path.includes(":")){const s=r.path.split("/"),i=t.split("/");if(s.length===i.length){let n=!0;for(let a=0;a<s.length;a++)if(s[a].startsWith(":"))e[s[a].slice(1)]=i[a];else if(s[a]!==i[a]){n=!1;break}return n}}return!1}),e}getCurrentPath(){return this.currentPath}}const h=new Y;document.addEventListener("DOMContentLoaded",async()=>{try{let c=function(){r.innerHTML="<view-home></view-home>",r.querySelector("view-home")?.addEventListener("navigate-to-day",n=>{h.navigate(`/day/${n.detail.dayId}`)})},t=function(){const n=h.getParams().id;if(n){r.innerHTML="<view-day></view-day>";const a=r.querySelector("view-day");a&&a.loadDay&&a.loadDay(n),a?.addEventListener("start-session",d=>{h.navigate(`/session/${d.detail.dayId}`)})}};await k.initialize();const e=document.querySelector("app-shell"),r=document.querySelector("#content");if(!r){console.error("Content container not found");return}h.register("/",()=>c()),h.register("/day/:id",()=>t()),h.register("/session/:id",()=>s());async function s(){const n=h.getParams().id;if(n){const a=await k.getDay(n);if(a){r.innerHTML="<view-session></view-session>";const d=r.querySelector("view-session");d&&d.setDay&&d.setDay(a),d?.addEventListener("session-complete",w=>{console.log("Session completed:",w.detail),h.navigate(`/day/${n}`)})}}}e&&(e.addEventListener("day-selected",i=>{const a=`p1_w1_d${i.detail.day}`;h.navigate(`/day/${a}`)}),e.addEventListener("open-settings",()=>{console.log("Open settings")}))}catch(c){console.error("Failed to initialize app:",c),document.body.innerHTML=`
    ...
    `}});
//# sourceMappingURL=index-R6iew4dX.js.map
