var F=Object.defineProperty;var B=(d,t,e)=>t in d?F(d,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):d[t]=e;var a=(d,t,e)=>B(d,typeof t!="symbol"?t+"":t,e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))r(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const n of i.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&r(n)}).observe(document,{childList:!0,subtree:!0});function e(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function r(s){if(s.ep)return;s.ep=!0;const i=e(s);fetch(s.href,i)}})();class A extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.setupEventListeners()}render(){this.shadowRoot&&(this.shadowRoot.innerHTML=`
      <style>
        :host {
          display: block;
          height: 100vh;
    ...
    `)}setupEventListeners(){if(!this.shadowRoot)return;this.shadowRoot.querySelectorAll(".day-btn").forEach(e=>{e.addEventListener("click",r=>{const i=r.target.dataset.day;i&&this.selectDay(i)})}),this.shadowRoot.querySelector(".settings-btn")?.addEventListener("click",()=>{this.dispatchEvent(new CustomEvent("open-settings"))})}selectDay(t){if(!this.shadowRoot)return;this.shadowRoot.querySelectorAll(".day-btn").forEach(r=>{r.classList.remove("active")}),this.shadowRoot.querySelector(`.day-btn[data-day="${t}"]`)?.classList.add("active"),this.dispatchEvent(new CustomEvent("day-selected",{detail:{day:t}}))}showTimer(t,e){if(!this.shadowRoot)return;const r=this.shadowRoot.querySelector("#timer-bar"),s=this.shadowRoot.querySelector("#timer-display"),i=this.shadowRoot.querySelector("#timer-round");r&&r.classList.add("active"),s&&(s.textContent=t),i&&(i.textContent=e)}hideTimer(){if(!this.shadowRoot)return;const t=this.shadowRoot.querySelector("#timer-bar");t&&t.classList.remove("active")}}customElements.define("app-shell",A);const z="modulepreload",W=function(d){return"/"+d},M={},q=function(t,e,r){let s=Promise.resolve();if(e&&e.length>0){let u=function(h){return Promise.all(h.map(l=>Promise.resolve(l).then(f=>({status:"fulfilled",value:f}),f=>({status:"rejected",reason:f}))))};var n=u;document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),c=o?.nonce||o?.getAttribute("nonce");s=u(e.map(h=>{if(h=W(h),h in M)return;M[h]=!0;const l=h.endsWith(".css"),f=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${h}"]${f}`))return;const m=document.createElement("link");if(m.rel=l?"stylesheet":z,l||(m.as="script"),m.crossOrigin="",m.href=h,c&&m.setAttribute("nonce",c),document.head.appendChild(m),l)return new Promise((C,E)=>{m.addEventListener("load",C),m.addEventListener("error",()=>E(new Error(`Unable to preload CSS for ${h}`)))})}))}function i(o){const c=new Event("vite:preloadError",{cancelable:!0});if(c.payload=o,window.dispatchEvent(c),!c.defaultPrevented)throw o}return s.then(o=>{for(const c of o||[])c.status==="rejected"&&i(c.reason);return t().catch(i)})};class N{constructor(){a(this,"db",null);a(this,"dbName","minimalift_v1");a(this,"version",1)}async openDb(){return this.db?this.db:new Promise((t,e)=>{const r=indexedDB.open(this.dbName,this.version);r.onerror=()=>e(r.error),r.onsuccess=()=>{this.db=r.result,t(this.db)},r.onupgradeneeded=s=>{const i=s.target.result;if(i.objectStoreNames.contains("programs")||i.createObjectStore("programs",{keyPath:"programId"}),i.objectStoreNames.contains("days")||i.createObjectStore("days",{keyPath:"dayId"}).createIndex("programId","programId",{unique:!1}),!i.objectStoreNames.contains("sessions")){const n=i.createObjectStore("sessions",{keyPath:"sessionId",autoIncrement:!0});n.createIndex("dayId","dayId",{unique:!1}),n.createIndex("date","startedAt",{unique:!1})}}})}async tx(t,e="readonly"){return(await this.openDb()).transaction([t],e).objectStore(t)}async get(t,e){const r=await this.tx(t);return new Promise((s,i)=>{const n=r.get(e);n.onsuccess=()=>s(n.result),n.onerror=()=>i(n.error)})}async getAll(t){const e=await this.tx(t);return new Promise((r,s)=>{const i=e.getAll();i.onsuccess=()=>r(i.result),i.onerror=()=>s(i.error)})}async put(t,e){const r=await this.tx(t,"readwrite");return new Promise((s,i)=>{const n=r.put(e);n.onsuccess=()=>s(n.result),n.onerror=()=>i(n.error)})}async delete(t,e){const r=await this.tx(t,"readwrite");return new Promise((s,i)=>{const n=r.delete(e);n.onsuccess=()=>s(),n.onerror=()=>i(n.error)})}async indexGetAll(t,e,r){const i=(await this.tx(t)).index(e);return new Promise((n,o)=>{const c=r?i.getAll(r):i.getAll();c.onsuccess=()=>n(c.result),c.onerror=()=>o(c.error)})}async clear(t){const e=await this.tx(t,"readwrite");return new Promise((r,s)=>{const i=e.clear();i.onsuccess=()=>r(),i.onerror=()=>s(i.error)})}}const v=new N,y=class y{constructor(){}static getInstance(){return y.instance||(y.instance=new y),y.instance}async initialize(){const{seedData:t}=await q(async()=>{const{seedData:r}=await import("./seed-data-C_uM7Nqm.js");return{seedData:r}},[]);await v.get("programs",t.programId)||await this.loadSeedData(t)}async loadSeedData(t){const e={programId:t.programId,title:t.title};await v.put("programs",e);for(const r of t.days)await v.put("days",r);console.log("Seed data loaded successfully")}async getProgram(t){return await v.get("programs",t)}async getDaysForProgram(t){return await v.indexGetAll("days","programId",t)}async getDay(t){return await v.get("days",t)}async getAllDays(){return(await v.getAll("days")).sort((e,r)=>e.order-r.order)}};a(y,"instance");let D=y;const S=D.getInstance();class U extends HTMLElement{constructor(){super();a(this,"days",[]);this.attachShadow({mode:"open"})}async connectedCallback(){this.days=await S.getAllDays(),this.render()}render(){this.shadowRoot&&(this.shadowRoot.innerHTML=`
      <style>
        :host {
          display: block;
          padding: 16px;
    ...
    `}}customElements.define("view-home",U);class H extends HTMLElement{constructor(){super();a(this,"day",null);this.attachShadow({mode:"open"})}async loadDay(e){this.day=await S.getDay(e)||null,this.render()}render(){if(this.shadowRoot){if(!this.day){this.shadowRoot.innerHTML="<p>Loading...</p>";return}this.shadowRoot.innerHTML=`
      <style>
        :host {
          display: block;
          padding: 16px;
    ...
    `:""}renderTimer(){const e=this.getCurrentExercise(),r=this.getCurrentBlock();if(!(r?.timerMode&&r.timerMode!=="none")&&e){const l=e.sets||1,f=this.currentSetIndex+1;if(this.isRestingBetweenSets&&this.currentTimer){const m=this.currentTimer.getRemainingInCurrentPeriod(),C=b.formatTime(m),E=this.currentTimer.getElapsedTime(),L=this.currentTimer.getDuration(),$=Math.min(E/L,1);return`
          <div class="timer-section">
            <div class="timer-display">${C}</div>
      ...
      `;const i=this.currentTimer.getRemainingInCurrentPeriod(),n=this.currentTimer.getCurrentRound(),o=this.currentTimer.getTotalRounds(),c=b.formatTime(i),u=this.getTimerProgress(),h=this.getTimerPhase();return`
      <div class="timer-section">
        <div class="timer-display">${c}</div>
    ...
    `}renderControls(){const e=this.getCurrentBlock(),r=this.getCurrentExercise();if(e?.timerMode&&e.timerMode!=="none"){const u=this.currentTimer?.getState()||"idle",h=u==="completed";let l="";return!this.currentTimer||u==="idle"?l='<button class="control-btn primary" id="start-btn">Start Timer</button>':u==="running"?l='<button class="control-btn warning" id="pause-btn">Pause</button>':u==="paused"?l='<button class="control-btn primary" id="resume-btn">Resume</button>':h&&(l='<button class="control-btn primary" id="next-btn">Next Exercise</button>'),`
      ...
      `}if(!r)return'<div class="session-controls"><button class="control-btn secondary" id="skip-btn">Skip</button></div>';const i=r.sets||1,n=this.isRestingBetweenSets,o=this.currentTimer?.getState()||"idle";if(n&&this.currentTimer){if(o==="running")return`
          <div class="session-controls">
    ...
    `}getCurrentBlock(){return!this.currentDay||this.currentBlockIndex>=this.currentDay.blocks.length?null:this.currentDay.blocks[this.currentBlockIndex]}getCurrentExercise(){const e=this.getCurrentBlock();return!e||this.currentExerciseIndex>=e.exercises.length?null:e.exercises[this.currentExerciseIndex]}getNextExercise(){const e=this.getCurrentBlock();return e?this.currentExerciseIndex+1<e.exercises.length?e.exercises[this.currentExerciseIndex+1]:!this.currentDay||this.currentBlockIndex+1>=this.currentDay.blocks.length?null:this.currentDay.blocks[this.currentBlockIndex+1].exercises[0]||null:null}formatExerciseDetails(e,r){const s=[];return e.sets&&s.push(`${e.sets} sets`),e.reps&&s.push(`${e.reps} reps`),e.restSec&&s.push(`${e.restSec}s rest`),r.timerMode&&r.timerMode!=="none"&&s.push(`${r.timerMode.toUpperCase()} format`),s.join(" • ")}getTimerProgress(){if(!this.currentTimer)return 0;const e=this.currentTimer.getElapsedTime(),r=this.currentTimer.getDuration();return Math.min(e/r,1)}getTimerPhase(){if(!this.currentTimer)return"Ready";const e=this.getCurrentBlock();if(!e?.timerMode||e.timerMode==="none")return"Timer Running";if(this.currentTimer instanceof T||this.currentTimer instanceof w){const r=this.currentTimer.getCurrentPhase?.();if(r==="work")return"Work Period";if(r==="rest")return"Rest Period"}return"Timer Running"}createTimerForCurrentBlock(){const e=this.getCurrentBlock();if(!e?.timerMode||e.timerMode==="none")return null;const r=e.rounds||e.exercises.length;switch(e.timerMode){case"emom":return x.createEMOM(r);case"e2mom":return x.createE2MOM(r);case"e4mom":return x.createE4MOM(r);case"n90":return T.create(r);case"timed_circuit":return w.createTabata(r);case"fixed_rest":const s=e.durationSec||180;return k.create(s);default:return null}}setupEventListeners(){this.shadowRoot&&this.shadowRoot.addEventListener("click",e=>{const r=e.target;r.id==="start-btn"?this.startTimer():r.id==="pause-btn"?this.pauseTimer():r.id==="resume-btn"?this.resumeTimer():r.id==="reset-btn"?this.resetTimer():r.id==="skip-btn"?this.skipExercise():r.id==="complete-exercise-btn"?this.completeExercise():r.id==="complete-set-btn"?this.completeSet():r.id==="skip-rest-btn"?this.skipRest():r.id==="next-set-btn"?this.startNextSet():r.id==="next-btn"?this.nextExercise():r.id==="finish-btn"?this.finishSession():r.id==="cancel-session"&&this.cancelSession()})}async startTimer(){this.sessionStartTime||(this.sessionStartTime=performance.now(),await P.requestNotificationPermission()),await R.acquire(),this.currentTimer=this.createTimerForCurrentBlock(),this.currentTimer&&(this.currentTimer.addCallback(e=>{this.handleTimerEvent(e)}),this.currentTimer.start(),p.timerStart(),this.render())}pauseTimer(){this.currentTimer?.pause(),p.timerPause(),this.render()}resumeTimer(){this.currentTimer?.start(),p.timerStart(),this.render()}resetTimer(){this.currentTimer?.reset(),this.render()}skipExercise(){this.nextExercise()}completeExercise(){this.nextExercise()}async completeSet(){const e=this.getCurrentExercise();if(!e)return;const r=e.sets||1,s=this.currentSetIndex>=r-1;p.setComplete(),s?this.nextExercise():(e.restSec&&e.restSec>0?(this.currentSetIndex++,this.isRestingBetweenSets=!0,this.currentTimer=k.create(e.restSec),this.currentTimer.addCallback(i=>{i.type==="complete"?this.handleRestComplete():i.type==="tick"&&this.updateTimerDisplay()}),this.currentTimer.start(),p.restStart()):this.currentSetIndex++,this.render())}skipRest(){this.currentTimer&&(this.currentTimer.stop(),this.currentTimer=null),this.handleRestComplete()}startNextSet(){this.handleRestComplete()}handleRestComplete(){this.isRestingBetweenSets=!1,this.currentTimer?.stop(),this.currentTimer=null,p.restComplete(),this.render()}nextExercise(){const e=this.getCurrentBlock();e&&(p.exerciseComplete(),this.currentTimer?.stop(),this.currentTimer=null,this.currentSetIndex=0,this.isRestingBetweenSets=!1,this.currentExerciseIndex+1<e.exercises.length?this.currentExerciseIndex++:(this.currentBlockIndex++,this.currentExerciseIndex=0),this.render())}async finishSession(){p.sessionComplete(),await this.cleanup(),this.showSessionSaveDialog()}showSessionSaveDialog(){const e=this.sessionStartTime?Math.round((performance.now()-this.sessionStartTime)/1e3):0,r=Math.floor(e/60),s=e%60,i=`${r}:${s.toString().padStart(2,"0")}`;if(!this.shadowRoot)return;const n=`
    ...
    `,o=document.createElement("div");o.innerHTML=n,this.shadowRoot.appendChild(o);const c=this.shadowRoot.querySelector("#discard-session"),u=this.shadowRoot.querySelector("#save-session");c?.addEventListener("click",()=>{this.discardSession()}),u?.addEventListener("click",()=>{this.saveSession()})}discardSession(){console.log("Session discarded (not saved)"),this.dispatchEvent(new CustomEvent("session-complete",{detail:{day:this.currentDay,duration:this.sessionStartTime?performance.now()-this.sessionStartTime:0,saved:!1}}))}saveSession(){console.log("Session saved"),this.dispatchEvent(new CustomEvent("session-complete",{detail:{day:this.currentDay,duration:this.sessionStartTime?performance.now()-this.sessionStartTime:0,saved:!0}}))}async cancelSession(){confirm("Are you sure you want to cancel this session? Your progress will not be saved.")&&(console.log("Session cancelled by user"),await this.cleanup(),this.dispatchEvent(new CustomEvent("session-complete",{detail:{day:this.currentDay,duration:this.sessionStartTime?performance.now()-this.sessionStartTime:0,saved:!1,cancelled:!0}})))}setupBackgroundHandling(){this.foregroundCallback=()=>{this.currentTimer?.getState()==="running"&&R.reacquire(),this.updateTimerDisplay()},P.addForegroundCallback(this.foregroundCallback)}async cleanup(){this.currentTimer&&(this.currentTimer.stop(),this.currentTimer=null),await R.release(),this.foregroundCallback&&(P.removeForegroundCallback(this.foregroundCallback),this.foregroundCallback=null)}handleTimerEvent(e){e.type==="tick"?(this.updateTimerDisplay(),this.checkForPhaseChange(),this.checkForCountdownWarning(e.remaining)):e.type==="complete"?(p.exerciseComplete(),this.render()):e.type==="roundComplete"&&p.roundComplete()}checkForPhaseChange(){const e=this.getTimerPhase();this.lastPhase&&this.lastPhase!==e&&(e==="Work Period"?p.workPeriodStart():e==="Rest Period"&&p.restPeriodStart()),this.lastPhase=e}checkForCountdownWarning(e){const r=Math.ceil(e/1e3);if(r<=3&&r>0){const s=this.lastWarnTime||0,i=performance.now();i-s>900&&(p.countdownWarning(),this.lastWarnTime=i)}}updateTimerDisplay(){if(!this.shadowRoot||!this.currentTimer)return;const e=this.shadowRoot.querySelector(".timer-display"),r=this.shadowRoot.querySelector(".progress-fill"),s=this.shadowRoot.querySelector(".timer-progress"),i=this.shadowRoot.querySelector(".timer-phase");if(e){const n=this.currentTimer.getRemainingInCurrentPeriod();e.textContent=b.formatTime(n)}if(r){const n=this.getTimerProgress();r.style.width=`${n*100}%`}if(s){const n=this.currentTimer.getCurrentRound(),o=this.currentTimer.getTotalRounds();s.textContent=`Round ${n} of ${o}`}i&&(i.textContent=this.getTimerPhase())}}customElements.define("view-session",Y);class G{constructor(){a(this,"registration",null);a(this,"showUpdateCallback",null);document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>this.setupUpdateDetection()):this.setupUpdateDetection()}async setupUpdateDetection(){if(!("serviceWorker"in navigator)){console.log("Service workers not supported");return}try{this.registration=await navigator.serviceWorker.register("/sw.js"),console.log("ServiceWorker registered:",this.registration),console.log("Checking for updates on startup..."),await this.checkForUpdates(),console.log("Checking for waiting service worker..."),this.checkForWaitingWorker(),setTimeout(()=>{console.log("Double-checking for waiting service worker after delay..."),this.checkForWaitingWorker()},1e3),this.registration.installing&&this.trackInstallingWorker(this.registration.installing),setInterval(()=>{document.visibilityState==="visible"&&(console.log("Periodic update check (30s interval)"),this.checkForUpdates())},3e4),document.addEventListener("visibilitychange",()=>{document.visibilityState==="visible"&&(console.log("App came to foreground, checking for updates"),this.checkForUpdates())}),this.registration.addEventListener("updatefound",()=>{console.log("Update found, installing new version...");const t=this.registration?.installing;t&&this.trackInstallingWorker(t)}),navigator.serviceWorker.addEventListener("controllerchange",()=>{console.log("New service worker took control, reloading page"),window.location.reload()})}catch(t){console.error("ServiceWorker registration failed:",t)}}async checkForUpdates(){if(!this.registration){console.log("No registration available for update check");return}try{console.log("Calling registration.update()..."),await this.registration.update(),console.log("Update check completed"),this.registration.waiting&&(console.log("Found waiting worker after update check"),this.showUpdateAvailable())}catch(t){console.error("Failed to check for updates:",t)}}checkForWaitingWorker(){this.registration&&this.registration.waiting&&(console.log("Found waiting service worker on startup"),this.showUpdateAvailable())}trackInstallingWorker(t){t.addEventListener("statechange",()=>{console.log("Service worker state changed:",t.state),t.state==="installed"&&navigator.serviceWorker.controller&&(console.log("New version ready"),this.showUpdateAvailable())})}showUpdateAvailable(){this.showUpdateCallback?this.showUpdateCallback():this.showDefaultUpdateNotification()}showDefaultUpdateNotification(){confirm("A new version of Minimalift is available! Click OK to update now.")&&this.applyUpdate()}onUpdateAvailable(t){this.showUpdateCallback=t}async applyUpdate(){if(!this.registration?.waiting){console.log("No update waiting");return}this.registration.waiting.postMessage({type:"SKIP_WAITING"})}async forceUpdateCheck(){console.log("Manual update check triggered..."),await this.checkForUpdates(),setTimeout(()=>{this.checkForWaitingWorker()},2e3)}}const I=new G;class K extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.setupUpdateListener()}connectedCallback(){this.render(),this.setupEventListeners()}setupUpdateListener(){I.onUpdateAvailable(()=>{this.show()})}render(){this.shadowRoot&&(this.shadowRoot.innerHTML=`
      <style>
        :host {
          position: fixed;
          bottom: 0;
    ...
    `)}setupEventListeners(){if(!this.shadowRoot)return;const t=this.shadowRoot.querySelector("#update"),e=this.shadowRoot.querySelector("#dismiss");t?.addEventListener("click",()=>{this.applyUpdate()}),e?.addEventListener("click",()=>{this.hide()})}show(){this.classList.add("visible")}hide(){this.classList.remove("visible")}async applyUpdate(){const t=this.shadowRoot?.querySelector("#update");t&&(t.textContent="Updating...",t.disabled=!0),await I.applyUpdate()}}customElements.define("update-notification",K);class _{constructor(){a(this,"routes",[]);a(this,"currentPath","");window.addEventListener("hashchange",()=>this.handleRouteChange()),window.addEventListener("DOMContentLoaded",()=>this.handleRouteChange())}register(t,e){this.routes.push({path:t,handler:e})}navigate(t){window.location.hash=t}handleRouteChange(){const t=window.location.hash.slice(1)||"/";this.currentPath=t;const e=this.routes.find(r=>r.path.includes(":")?new RegExp("^"+r.path.replace(/:[^/]+/g,"([^/]+)")+"$").test(t):r.path===t);if(e)e.handler();else{const r=this.routes.find(s=>s.path==="/");r&&r.handler()}}getParams(){const t=window.location.hash.slice(1)||"/",e={};return this.routes.find(r=>{if(r.path.includes(":")){const s=r.path.split("/"),i=t.split("/");if(s.length===i.length){let n=!0;for(let o=0;o<s.length;o++)if(s[o].startsWith(":"))e[s[o].slice(1)]=i[o];else if(s[o]!==i[o]){n=!1;break}return n}}return!1}),e}getCurrentPath(){return this.currentPath}start(){this.handleRouteChange()}}const g=new _;document.addEventListener("DOMContentLoaded",async()=>{console.log("DOM loaded, initializing app...");try{let d=function(){r.innerHTML="<view-home></view-home>",r.querySelector("view-home")?.addEventListener("navigate-to-day",n=>{g.navigate(`/day/${n.detail.dayId}`)})},t=function(){const n=g.getParams().id;if(n){r.innerHTML="<view-day></view-day>";const o=r.querySelector("view-day");o&&o.loadDay&&o.loadDay(n),o?.addEventListener("start-session",c=>{g.navigate(`/session/${c.detail.dayId}`)})}};console.log("Initializing program manager..."),await S.initialize(),console.log("Program manager initialized");const e=document.querySelector("app-shell");if(!e){console.error("App shell not found");return}console.log("Found app shell, waiting for shadow DOM..."),await new Promise(i=>{const n=()=>{const o=e.shadowRoot;o&&o.querySelector("#content")?(console.log("Shadow DOM ready"),i(void 0)):setTimeout(n,10)};n()});const r=e.shadowRoot.querySelector("#content");if(!r){console.error("Content container not found");return}g.register("/",()=>d()),g.register("/day/:id",()=>t()),g.register("/session/:id",()=>s());async function s(){const n=g.getParams().id;if(n){const o=await S.getDay(n);if(o){r.innerHTML="<view-session></view-session>";const c=r.querySelector("view-session");c&&c.setDay&&c.setDay(o),c?.addEventListener("session-complete",u=>{console.log("Session completed:",u.detail),g.navigate(`/day/${n}`)})}}}e&&(e.addEventListener("day-selected",i=>{const o=`p1_w1_d${i.detail.day}`;g.navigate(`/day/${o}`)}),e.addEventListener("open-settings",()=>{console.log("Open settings")})),console.log("Starting router..."),g.start(),!window.location.hash||window.location.hash==="#/"?(console.log("No hash found, navigating to Day 1"),g.navigate("/day/p1_w1_d1")):console.log("Found hash:",window.location.hash),console.log("App initialization complete")}catch(d){console.error("Failed to initialize app:",d),document.body.innerHTML=`
    ...
    `}});
//# sourceMappingURL=index-CD746y9H.js.map
