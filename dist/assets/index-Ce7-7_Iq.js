var L=Object.defineProperty;var F=(c,t,e)=>t in c?L(c,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):c[t]=e;var a=(c,t,e)=>F(c,typeof t!="symbol"?t+"":t,e);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))r(i);new MutationObserver(i=>{for(const s of i)if(s.type==="childList")for(const n of s.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&r(n)}).observe(document,{childList:!0,subtree:!0});function e(i){const s={};return i.integrity&&(s.integrity=i.integrity),i.referrerPolicy&&(s.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?s.credentials="include":i.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function r(i){if(i.ep)return;i.ep=!0;const s=e(i);fetch(i.href,s)}})();class $ extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.setupEventListeners()}render(){this.shadowRoot&&(this.shadowRoot.innerHTML=`
    ...
    `,this.shadowRoot.querySelectorAll(".day-card").forEach(e=>{e.addEventListener("click",r=>{const i=r.currentTarget.dataset.dayId;i&&this.dispatchEvent(new CustomEvent("navigate-to-day",{detail:{dayId:i},bubbles:!0,composed:!0}))})}))}renderDayCard(e){const r=[...new Set(e.blocks.map(s=>s.type))],i=e.blocks.reduce((s,n)=>s+n.exercises.length,0);return`
      <div class="day-card" data-day-id="${e.dayId}">
        <div class="day-number">${e.order}</div>
        <div class="day-title">${e.title}</div>
        <div class="day-summary">${i} exercises • ${e.blocks.length} blocks</div>
    ...
    `,this.shadowRoot.querySelector(".start-session-btn")?.addEventListener("click",e=>{const i=e.target.dataset.dayId;i&&this.dispatchEvent(new CustomEvent("start-session",{detail:{dayId:i},bubbles:!0,composed:!0}))}),this.shadowRoot.querySelectorAll(".substitute-chip").forEach(e=>{e.addEventListener("click",r=>{const i=r.target,s=i.dataset.exerciseId,n=i.dataset.substitute;s&&n&&console.log("Switch to substitute:",n,"for exercise:",s)})})}}renderBlock(e){const r=this.getTimerLabel(e.timerMode);return`
      <div class="block">
        ...
        ${e.exercises.map(i=>this.renderExercise(i)).join("")}
    ...
    `:this.renderSessionComplete()}renderHeader(){if(!this.currentDay)return"";const e=this.currentDay.blocks.length,r=this.currentDay.blocks.reduce((s,n)=>s+n.exercises.length,0),i=this.currentBlockIndex*(this.currentDay.blocks[0]?.exercises.length||0)+this.currentExerciseIndex;return`
    ...
    `:""}renderTimer(){if(!this.currentTimer)return`
      ...
      `;const e=this.currentTimer.getRemainingInCurrentPeriod(),r=this.currentTimer.getCurrentRound(),i=this.currentTimer.getTotalRounds(),s=b.formatTime(e),n=this.getTimerProgress(),o=this.getTimerPhase();return`
      <div class="timer-section">
        <div class="timer-display">${s}</div>
        <div class="timer-phase">${o}</div>
        <div class="timer-progress">Round ${r} of ${i}</div>
        <div class="progress-bar">
          <div class="progress-fill" style="width: ${n*100}%"></div>
        </div>
      </div>
    `}renderControls(){const e=this.currentTimer?.getState()||"idle",r=e==="completed";let i="";return!this.currentTimer||e==="idle"?i='<button class="control-btn primary" id="start-btn">Start Timer</button>':e==="running"?i='<button class="control-btn warning" id="pause-btn">Pause</button>':e==="paused"?i='<button class="control-btn primary" id="resume-btn">Resume</button>':r&&(i='<button class="control-btn primary" id="next-btn">Next Exercise</button>'),`
    ...
    `}});
//# sourceMappingURL=index-Ce7-7_Iq.js.map
